...
def calculate_trading_metrics(
    trades_df, btc_price_df=None, time_filter=None, filter_time=None
):
    if trades_df.empty:
        return {
            "total_return": 0,
    ...
    # 초기 투자 금액 추정
    initial_investment = (
        closed_trades.sort_values("timestamp").head(3)["entry_price"].mean()
    ...
    if btc_price_df is not None and not btc_price_df.empty:
        if time_filter != "전체" and filter_time is not None:
            ...
            first_trade_time = closed_trades.sort_values("timestamp").iloc[0][
                "timestamp"
            ]
            last_trade_time = closed_trades.sort_values("timestamp").iloc[-1][
                (
                    "timestamp"
                    if "exit_timestamp" not in closed_trades.columns
                    else "exit_timestamp"
                )
            ]
    ...
    total_loss = abs(
        closed_trades[closed_trades["profit_loss"] < 0]["profit_loss"].sum()
    )
    ...
    if "profit_loss_percentage" in closed_trades.columns and len(closed_trades) > 1:
        ...
        sharpe_ratio = (
            (returns.mean() / returns.std()) * np.sqrt(365) if returns.std() > 0 else 0
        )
    else:
        sharpe_ratio = 0
    ...
    # 평균 보유 시간
    if (
        "exit_timestamp" in closed_trades.columns
        and "timestamp" in closed_trades.columns
    ):
        # timestamp와 exit_timestamp가 모두 datetime인지 확인
        valid_timestamps = closed_trades.dropna(subset=["exit_timestamp"])
        if not valid_timestamps.empty:
            holding_times = (
                valid_timestamps["exit_timestamp"] - valid_timestamps["timestamp"]
...
try:
    ...
    time_filter = st.sidebar.selectbox(
        "기간 선택:", ["전체", "최근 24시간", "최근 7일", "최근 30일", "최근 90일"]
    )
    ...
    metrics = calculate_trading_metrics(
        filtered_trades, btc_price_df, time_filter, filter_time
    )
    ...
    # 현재 BTC 가격
    current_btc_price = (
        ai_analysis_df.iloc[0]["current_price"]
        if not ai_analysis_df.empty
        else btc_price_df.iloc[-1]["close"]
    )

    # 대시보드 메인
    st.markdown(
        "<h1 class='header'>Bitcoin Trading Dashboard</h1>", unsafe_allow_html=True
    )
    ...
    with position_cols[1]:
        position_status = (
            ...
            else f"{current_position['action'].upper()}"
        )
        position_color = (
            ""
            if not has_open_position
            else ("positive" if current_position["action"] == "long" else "negative")
        )
        st.markdown(
            f"""
        <div class="metric-card" style="width: 100%">
    ...
    # BTC 가격 차트와 거래 시점 표시
    st.markdown(
        "<h2 class='subheader'>Bitcoin Price Chart & Trade Entries</h2>",
        unsafe_allow_html=True,
    )

    # BTC 차트 기간 필터링
    filtered_price_df = btc_price_df[
        btc_price_df["timestamp"] > (now - timedelta(days=chart_days))
    ]
    ...
    # 청산 포인트
    exit_points = filtered_trades[
        (filtered_trades["status"] == "CLOSED")
        & (filtered_trades["exit_price"].notna())
    ]
    if not exit_points.empty:
        fig.add_trace(
            go.Scatter(
                x=(
                    exit_points["exit_timestamp"]
                    if "exit_timestamp" in exit_points.columns
                    else exit_points["timestamp"]
                ),
    ...
    # 거래 성과 차트
    st.markdown(
        "<h2 class='subheader'>Trading Performance</h2>", unsafe_allow_html=True
    )
    ...
    if has_open_position:
        st.markdown(
            "<h2 class='subheader'>Current Open Position</h2>", unsafe_allow_html=True
        )
        ...
        with position_cols[1]:
            ...
            if isinstance(current_btc_price, (int, float)):
                ...
                price_color = (
                    "green"
                    if (current_position["action"] == "long" and price_diff > 0)
                    or (current_position["action"] == "short" and price_diff < 0)
                    else "red"
                )
    ...
    if not ai_analysis_df.empty:
        ...
        with analysis_cols[0]:
            direction_color = (
                "green"
                if latest_analysis["direction"] == "LONG"
                else "red" if latest_analysis["direction"] == "SHORT" else "orange"
            )
            st.markdown(
                f"""
            ### AI Analysis Summary
        ...
        with analysis_cols[1]:
            ...
            # 분석 내용 일부만 표시
            reasoning_preview = (
                latest_analysis["reasoning"][:200] + "..."
                if len(latest_analysis["reasoning"]) > 200
                else latest_analysis["reasoning"]
            )
